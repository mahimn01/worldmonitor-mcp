...
Every score ships with its factor breakdown, per-feed coverage, and a confidence number; scores are withheld when coverage is too low. Feeds that fail degrade to coverage flags, never fabricated factors. All of it is read-only and informational, not investment advice.
